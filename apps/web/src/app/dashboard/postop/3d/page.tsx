...
import { getCasesForPlanning, uploadFile, createMeasurement, triggerPlan, plan3dMulti } from '@/lib/api';

const STEPS = ['术后影像导入', '二维映射', '畸形相关测量分析', '矫正数据输入', '矫正路径规划'];

export default function Postop3DPage() {
  ...
  const [planResult, setPlanResult] = useState<{ plan_total_days?: number } | null>(null);
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setLoading(true);
    try {
      await uploadFile(selectedCaseId, file, 'STL');
      setMsg('上传成功');
    } catch (err) {
  ...
  async function handleSaveMeasurement() {
    ...
    setLoading(true);
    try {
      await createMeasurement({ caseId: selectedCaseId, stage: 'POSTOP_3D', values: measureValues });
      setMsg('测量数据已保存');
    } catch (e) {
  ...
  async function handleTriggerPlan() {
    ...
    setMsg('');
    try {
      const r = await triggerPlan(selectedCaseId, '3d', { goalMm: [goalMm[0], goalMm[1], goalMm[2]] });
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="page-description">二级：三维术后规划</p>
      </div>
      ...
      {step === 0 && (
        <div className="card-medical">
          ...
          <input type="file" accept=".stl" onChange={handleUpload} disabled={loading} className="mt-2 block text-sm" />
      ...
      {step === 4 && (
        <div className="card-medical">
          ...
          <p className="mt-1 text-sm text-medical-muted">三维 A* 算法生成整体方案</p>
          <button type="button" onClick={handleTriggerPlan} disabled={loading} className="btn-primary mt-3">
            {loading ? '规划中…' : '生成矫正方案'}
          </button>
          {planResult && <p className="mt-2 text-sm">总天数: {planResult.plan_total_days ?? '—'}</p>}
          {msg && <p className="mt-2 text-sm">{msg}</p>}
        </div>
      )}

      <div className="mt-6">
        <Link href="/dashboard/postop/2d" className="text-sm text-medical-primary hover:underline">
          ← 切换到二维术后规划
        </Link>
      </div>
    </div>
